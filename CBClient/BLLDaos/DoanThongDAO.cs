...
{
    public class DoanThongDAO
    {
        #region Tinh_DoanThong
        public async static Task<DoanThong> bindDoanThongToCoBao(CoBao rowCoBao)
        {
            DoanThong dtInfo = new DoanThong();
        ...
        public static List<DoanThongDM> bindDoanThongDM(CoBao cobao, List<CoBaoDM> listcobaodm)
        {
            List<DoanThongDM> listDTDM = new List<DoanThongDM>();
            try
            {
                if (listcobaodm.Count > 0)
                {
                   foreach(CoBaoDM cbdm in listcobaodm)
                    {
                        DoanThongDM dtdm = new DoanThongDM();
        ...
        public static List<DoanThongCT> bindDoanThongCT(int dungKB, CoBao cobao, List<CoBaoCT> listcobaoct)
        {
            List<DoanThongCT> listDTCT = new List<DoanThongCT>();
            CoBaoCT cbct = new CoBaoCT();
            try
            {
                //Trường hợp tác nghiệp bãi bỏ không có cơ báo chi tiết
                if(listcobaoct.Count == 0)
                {
                    DoanThongCT doanThongct = new DoanThongCT();
                ...
                {                    
                   ...
                   var listDTCTCN = fnNapDoanThongCT(dungKB, cobao, listcobaoct,cbct);
                    //Duyệt lại đoan thống chi tiết để gán lại tuyến khu đoạn chung tuyến
                    DoanThongCT ctOld = new DoanThongCT();
                    bool isFisrt = true;
                    foreach (DoanThongCT ct in listDTCTCN)
                    {
                        ...
                        {
                            var tuyenTau = listDTCTCN.Where(x => x.MacTauID == ct.MacTauID && x.GaXPID==ct.GaXPID && x.GaKTID != ct.GaXPID).FirstOrDefault();
                            if (tuyenTau != null)
                            {
                    ...
                    listDTCT = (from ct in listDTCTCN
                                group ct by new { ct.TauID,ct.MacTauID, ct.TinhChatID, ct.TuyenID,ct.KhuDoan,ct.MayGhepID, ct.XeTotal, ct.TanXeRong, ct.XeRongTotal } into g
                                       ...
                                       }).OrderBy(x => x.NgayXP).ToList<DoanThongCT>();                   
            ...
            {
                throw new Exception("Lỗi tính đoạn thống chi tiết cơ báo số:" + cobao.SoCB + ". "+ ex.Message);
            }
            return listDTCT;
        }
        private static List<DoanThongCT> fnNapDoanThongCT(int dungKB, CoBao cobao,List<CoBaoCT> listcobaoct,CoBaoCT cbct)
        {
            List<DoanThongCT> listdoanthongct = new List<DoanThongCT>();
            DoanThongCT doanThongct = new DoanThongCT();
            List<DoanThongCT> listTemp = new List<DoanThongCT>();
            var gadoanminh = AppGlobal.DonviDMList.Where(x => x.MaDV == cobao.DvcbID).FirstOrDefault().GaDMList;
            CoBaoCT rowPrev = null;
            CoBaoCT rowNext = null;
            int soTT = 1;
            ...
            foreach (CoBaoCT row in listcobaoct)
            {
                ...
                doanThongct.CongTacID = AppGlobal.LoaitauList.Where(x=>x.LoaiTauID==row.CongTacID).FirstOrDefault().CongTacID;
                ...
                if (soTT == 1)
                {
                    doanThongct.DonXP += (int)row.GioDon;                    
                    doanThongct.DungXP += (int)(row.GioDi - cobao.RaKho).TotalMinutes - (int)row.GioDon;
                    if(listcobaoct.Count==1)
                        doanThongct.DungKT = (int)(cobao.VaoKho - row.GioDi).TotalMinutes;
                }
                else if (soTT == listcobaoct.Count)
                {
                    doanThongct.DonKT += (int)row.GioDon;
                    ...
                    doanThongct.DungKT -= (int)row.GioDon;
                }
                ...
                else
                {
                    ...
                    if (soTT == 2 && (rowPrev.MacTauID == "CDON" || rowPrev.MacTauID == "KDON") && row.MacTauID != "CDON" && row.MacTauID != "KDON")
                    {
                        ...
                        doanThongct.DungXP -= (int)row.GioDon;
                    }
                    else
                    {
                        doanThongct.DonDD += (int)row.GioDon;
                        ...
                        doanThongct.DungDD -= (int)row.GioDon;
                    }
                }               
                ...
                //Nếu là máy ghép nguội hoặc đẩy nguội thì KM=0
                if (row.TinhChatID == 4)
                {
                    tanChia = 0;
                    tkmChay = 0;
                }                
                else if (row.TinhChatID == 3)
                {
                    tanChia = 0;
                    tkmChay = 0;
                }
                    ...
                    {                        
                        ...
                        if (congLenhSK != null)
                        {
                            ...
                            if (congLenhSK.ID == 26)//Lên dốc khe nét
                            {
                                ...
                                tkmChay = doanThongct.TinhChatID <= 2 && kmChay > kmThem ? (kmChay - kmThem) * tanChia : 0;
            ...
            foreach (DoanThongCT ct in listTemp)
            {
                if (ct.GaXPID == ct.GaKTID && (ct.CongTacID!=8||ct.CongTacID!=9))
                {
        ...
        private static void fnPhanBoGio(CoBao cobao, ref List<DoanThongCT> listdoanthongct)
        {            
            ...
            foreach (DoanThongCT row in listdoanthongct)
            {               
            ...
            DoanThongCT rowC = listdoanthongct.OrderByDescending(x=>x.DonThuan).FirstOrDefault();
        ...
        public static decimal fnHeSoChayDon(string data)
        {
            decimal heSo = 0;
            switch (data)
            {
                case "EM": heSo = (decimal)0.7; break;
                case "D4H": heSo = (decimal)0.5; break;
                case "D5H": heSo = (decimal)0.5; break;
                case "D8E": heSo = (decimal)0.5; break;
                case "D9E": heSo = (decimal)0.8; break;
                case "D10H": heSo = (decimal)0.85; break;
                case "D11H": heSo = (decimal)0.85; break;
                case "D12E": heSo = (decimal)0.87; break;
                case "D13E": heSo = (decimal)0.87; break;
                case "D14Er": heSo = (decimal)0.85; break;
                case "D18E": heSo = (decimal)0.87; break;
                case "D19E": heSo = (decimal)0.87; break;
                case "D19Er": heSo = (decimal)0.87; break;
                case "D20E": heSo = (decimal)0.87; break;
            }
            return heSo;
        }
        public static decimal fnHeSoDon(string data)
        {
            decimal heSo = 0;
            switch (data)
            {
                case "EM": heSo = (decimal)5; break;
                case "D4H": heSo = (decimal)5; break;
                case "D5H": heSo = (decimal)5; break;
                case "D8E": heSo = (decimal)5; break;
                case "D9E": heSo = (decimal)16; break;
                case "D10H": heSo = (decimal)16; break;
                case "D11H": heSo = (decimal)16; break;
                case "D12E": heSo = (decimal)16; break;
                case "D13E": heSo = (decimal)16; break;
                case "D14Er": heSo = (decimal)18; break;
                case "D18E": heSo = (decimal)18; break;
                case "D19E": heSo = (decimal)18; break;
                case "D19Er": heSo = (decimal)18; break;
                case "D20E": heSo = (decimal)18; break;
            }
            return heSo;
        }
        public static List<DoanThongCT> fnPhanBoNhienLieu(CoBao cobao, List<DoanThongCT> listdoanthongctOld)
        {
            List<DoanThongCT> listdoanthongct = new List<DoanThongCT>();
            ...
            {
                foreach (DoanThongCT row in listdoanthongDB)
                {
                    //Tinh NL                
                    if (NLTieuThuRunning < _NLTieuThu && row.TinhChatID == 2)
                    {
                        decimal nlTieuThu = row.KMDon > 0 ? Math.Round(fnHeSoChayDon(cobao.LoaiMayID) * row.KMDon, 2) : 0;
                    ...
                    else if (NLTieuThuRunning < _NLTieuThu && (row.CongTacID == 8 || row.CongTacID == 9))
                    {
                        ...
                        decimal nlTieuThu = gioDon > 0 ? Math.Round(fnHeSoDon(cobao.LoaiMayID) * gioDon, 2) : 0;
                        nlTieuThu = _NLTieuThu - NLTieuThuRunning > nlTieuThu ? nlTieuThu : _NLTieuThu - NLTieuThuRunning;
                        NLTieuThuRunning += nlTieuThu;
                        row.TieuThu += nlTieuThu;
                    }
            ...
            if (listdoanthongBT.Count>0)//Nếu có kéo tầu có tấn số
            {
                ...
                foreach (DoanThongCT row in listdoanthongBT)
                {
            ...
            //Cộng bù còn lại cho bản ghi cuối
            DoanThongCT rowC = listdoanthongct.OrderByDescending(x=>x.TKMChinh+x.TKMDon+x.TKMGhep+x.TKMDay).FirstOrDefault();
            if (rowC != null)
            {
