...
{
    public partial class BCGioDonForm : DevComponents.DotNetBar.Metro.MetroForm
    {   
        ...
        public BCGioDonForm()
        {
            ...
            string[] arRayDons = new string[] { "Tất cả", "Chuyên dồn", "Kiêm Dồn" };
            cboCongTacDon.Items.AddRange(arRayDons);
            cboCongTacDon.SelectedIndex = 0;           
        ...
        private void btnTraTim_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int loaiDon = cboCongTacDon.SelectedIndex;               
                int nguonDL = cboNguondl.SelectedIndex;
                List<BCGioDonInfo> list = new List<BCGioDonInfo>();                
                int TongSoBG = 0;
                //Lấy dữ liệu
                BaoCaoDAO.NapBCGioDon(nguonDL,maDV,loaiDon, cboLoaiBC.SelectedIndex, sdNgayBD.Value, sdNgayKT.Value, ref TongSoBG, ref list);                
                ...
                FormHelper.ShowReport(reportViewer1, "RptGioDon", "BaoCaoDS", list, rptParamList);
