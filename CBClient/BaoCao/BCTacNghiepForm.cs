...
{
    public partial class BCTacNghiepForm : DevComponents.DotNetBar.Metro.MetroForm
    {   
        string loaiBC = string.Empty;       
        public BCTacNghiepForm()
        {
            ...
            string[] arRayDons = new string[] { "Theo cơ báo", "Theo đầu máy" };
            cboCongTac.Items.AddRange(arRayDons);
            cboCongTac.SelectedIndex = 0;
        ...
        private void BCTacNghiepForm_Load(object sender, EventArgs e)
        {
        ...
        private void btnTraTim_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int TongSoBG = 0;
                //Lấy dữ liệu
                if(cboCongTac.SelectedIndex==0)
                    BaoCaoDAO.NapBCDCSPTNKT(nguonDL,maDV, cboLoaiBC.SelectedIndex, sdNgayBD.Value, sdNgayKT.Value, ref TongSoBG, ref list);
                else
                    BaoCaoDAO.NapBCDCSPTNQL(nguonDL,maDV, cboLoaiBC.SelectedIndex, sdNgayBD.Value, sdNgayKT.Value, ref TongSoBG, ref list);
                ...
                string tenBC = "giữa tổng công ty đường sắt việt nam và chi nhánh xí nghiệp "+ cboDonVi.Text.Split('.')[1];
                rptParam = new ReportParameter("prmNhombc", tenBC.ToUpper());
                rptParamList.Add(rptParam);
                ...
                rptParam = new ReportParameter("prmXinghiep", cboDonVi.Text.Split('.')[1].ToUpper());
                rptParamList.Add(rptParam);
                ...
                rptParamList.Add(rptParam);
                if(cboCongTac.SelectedIndex==0)
                    FormHelper.ShowReport(reportViewer1, "RptTacNghiepKT", "BaoCaoDS", list, rptParamList);
                else
                    FormHelper.ShowReport(reportViewer1, "RptTacNghiepQL", "BaoCaoDS", list, rptParamList);              
