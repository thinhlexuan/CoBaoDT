...
{
    public partial class HNKhuDoanForm : DevComponents.DotNetBar.Metro.MetroForm
    {       
        bool bThem = false;
        public HNKhuDoanForm()
        {
        ...
        private void fnTraTim()
        {
            try
            {
                ...
                List<HNKhuDoan> listHNKhudoan = HttpHelper.GetList<HNKhuDoan>(Configuration.UrlCBApi + "api/HaNois/HNGetKhuDoan" + data)
                   .OrderBy(x=>x.LoaiMayID).ToList();                
                if (listHNKhudoan.Count <= 0)
                {
                    throw new Exception("Không có dữ liệu.");

                }
                bsKhuDoan.DataSource = listHNKhudoan;
                dataGridView1.Refresh();
                lblTableCount.Text = "Tổng số bản ghi:" + listHNKhudoan.Count.ToString("N0");
        ...
        private void ShowControl(bool b)
        {
            ...
            txtKhuDoanID.Enabled = b;
            txtKhuDoanName.Enabled = b;
            sdNgayHL.Enabled = b;
        ...
        private void ClearControl()
        {
            ...
            txtKhuDoanID.ResetText();
            txtKhuDoanName.ResetText();            
        ...
        {
            HNKhuDoan kd = bsKhuDoan.Current as HNKhuDoan;
            if (kd != null)
            {
                ...
                txtKhuDoanID.Text = kd.KhuDoanID;
                txtKhuDoanName.Text = kd.KhuDoanName;
                sdNgayHL.Value = kd.NgayHL;
        ...
        private HNKhuDoan BindObject()
        {
            HNKhuDoan kd = new HNKhuDoan();
            if (!bThem)
                kd = bsKhuDoan.Current as HNKhuDoan;
            ...
            kd.KhuDoanID = txtKhuDoanID.Text;
            kd.KhuDoanName = txtKhuDoanName.Text;
            kd.NgayHL = sdNgayHL.Value;
        ...
        private void TinhCacGa_KM(HNKhuDoan kd)
        {
            kd.Km = 0;
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                var opStatus = HttpHelper.Delete<HNKhuDoan>(Configuration.UrlCBApi + "api/HaNois/HNDeleteKhuDoan?id=" + kd.ID);
        ...
        private async void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bThem)
                {
                    ...
                    var objInsert = await HttpHelper.Post<HNKhuDoan>(Configuration.UrlCBApi + "api/HaNois/HNPostKhuDoan", kd);                   
                    kd.ID = objInsert.ID;
                    bsKhuDoan.Add(kd);
                    bsKhuDoan.MoveLast();
                }
                else
                {
                    ...
                    kd.Modifyby = AppGlobal.User.Username;
                    kd.ModifyName = AppGlobal.User.FullName;
                    var objUpdate = await HttpHelper.Put<HNKhuDoan>(Configuration.UrlCBApi + "api/HaNois/HNPutKhuDoan?id=" + kd.ID, kd);
                    bsKhuDoan.EndEdit();
                }
            }
            catch (Exception ex)
