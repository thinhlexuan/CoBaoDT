...
{
    public partial class VIDMDinhMucForm : DevComponents.DotNetBar.Metro.MetroForm
    {       
        bool bThem = false;
        public VIDMDinhMucForm()
        {
        ...
        private void fnTraTim()
        {
            try
            {
                ...
                List<VIDMDinhMuc> listVIDMDinhMuc = HttpHelper.GetList<VIDMDinhMuc>(Configuration.UrlCBApi + "api/Vinhs/VIGetDMDinhMuc" + data)
                   .OrderBy(x=>x.LoaiMayID).ToList();                
                if (listVIDMDinhMuc.Count <= 0)
                {
                    throw new Exception("Không có dữ liệu.");

                }
                bsDMDinhMuc.DataSource = listVIDMDinhMuc;
                dataGridView1.Refresh();
                lblTableCount.Text = "Tổng số bản ghi:" + listVIDMDinhMuc.Count.ToString("N0");
        ...
        {
            VIDMDinhMuc dm = bsDMDinhMuc.Current as VIDMDinhMuc;
            if (dm != null)
            {
        ...
        private VIDMDinhMuc BindObject()
        {
            VIDMDinhMuc dm = new VIDMDinhMuc();
            if (!bThem)
                dm = bsDMDinhMuc.Current as VIDMDinhMuc;
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                var opStatus = HttpHelper.Delete<VIDMDinhMuc>(Configuration.UrlCBApi + "api/Vinhs/VIDeleteDMDinhMuc?id=" + dm.ID);
        ...
        private async void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bThem)
                {
                    ...
                    var objInsert = await HttpHelper.Post<VIDMDinhMuc>(Configuration.UrlCBApi + "api/Vinhs/VIPostDMDinhMuc", dm);
                    //fnTraTim();
                    dm.ID = objInsert.ID;
                    bsDMDinhMuc.Add(dm);
                    bsDMDinhMuc.MoveLast();
                ...
                else
                {
                    ...
                    dm.Modifyby = AppGlobal.User.Username;
                    dm.ModifyName = AppGlobal.User.FullName;
                    var objUpdate = await HttpHelper.Put<VIDMDinhMuc>(Configuration.UrlCBApi + "api/Vinhs/VIPutDMDinhMuc?id=" + dm.ID, dm);
                    bsDMDinhMuc.EndEdit();
                }
            }
            catch (Exception ex)
