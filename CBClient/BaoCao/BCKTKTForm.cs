...
{
    public partial class BCKTKTForm : DevComponents.DotNetBar.Metro.MetroForm
    {   
        ...
        private void btnTraTim_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //Lấy dữ liệu
                    BaoCaoDAO.NapBCKTKTTH(nguonDL,maDV, cboLoaiBC.SelectedIndex, sdNgayBD.Value, sdNgayKT.Value, loaiMay,strLoaiMay, strTuyen, ref TongSoBG, ref listTH);
                ...
                {
                    //Lấy dữ liệu
                    BaoCaoDAO.NapBCKTKTXN(nguonDL,maDV, cboLoaiBC.SelectedIndex, sdNgayBD.Value, sdNgayKT.Value, loaiMay,strLoaiMay, strTuyen, ref TongSoBG, ref listXN);
                    //Kiểm tra xem có dữ liệu hay không
                    if (listXN.Count <= 0)
                        throw new Exception("Không có dữ liệu.");
                }
                ...
                if (maDV == "TCT" || maDV == "UB")
                {
                    FormHelper.ShowReport(reportViewer1, "RptKTKTTH", "BaoCaoDS", listTH, rptParamList);
                }
                else
                {
                    FormHelper.ShowReport(reportViewer1, "RptKTKTXN", "BaoCaoDS", listXN, rptParamList);
                }
