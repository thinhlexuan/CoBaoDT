...
{
    public class DinhMucGADAO
    {
        #region YV_DinhMuc       
        public static void YVNapNLDinhMuc(CoBaoGA rowCoBao, List<CoBaoGACT> listCoBaoCT, List<DoanThongGACT> listdoanthongct)
        {            
            try
            {
        ...
        public static void YVNapDMDinhMuc(CoBaoGA rowCoBao, decimal kmTotal, List<DoanThongGADM> listdoanthongdm)
        {
            try
            {
                if (rowCoBao.DvcbID == "YV")
                {
                    foreach (DoanThongGADM ct in listdoanthongdm)
                    {
        ...
        #region HN_DinhMuc       
        public static void HNNapNLDinhMuc(CoBaoGA rowCoBao, List<DoanThongGACT> listdoanthongct)
        {
            try
            {
                if (rowCoBao.DvcbID == "HN")
                {
                    foreach (DoanThongGACT ct in listdoanthongct)
                    {
                        ...
                        {
                            //Nạp định mức có tính chất khác ghgeps nguội và không phải là tác nghiệp bãi bỏ
                            if (ct.TinhChatID != 4 && !string.IsNullOrWhiteSpace(ct.MacTauID))
                            {
                                ...
                                if (dmnl != null)
                                {
                                    ...
                                    //Cộng thêm định mức rút giờ nhiên liệu 
                                    if (ct.RutGioNL != 0)
                                    {
                                        ct.DinhMuc += (decimal)(dmnl.HeSoC * tanBQ * ct.RutGioNL);
                                        if (!string.IsNullOrWhiteSpace(ct.DienGiai)) ct.DienGiai += "-";
                                        ct.DienGiai += "Rút giờ LN=" + dmnl.HeSoC + "*" + tanBQ.ToString("N2") + "*" + ct.RutGioNL;
                                    }
                                }
                    ...
                    if (dtGhepNguoi != null)
                    {
                        ...
                        //Nếu có ghép nguội và làm các tác nghiệp khác
                        else
                        {
        ...
        public static void HNNapDMDinhMuc(CoBaoGA rowCoBao, decimal kmTotal, List<DoanThongGADM> listdoanthongdm)
        {
            try
            {
                if (rowCoBao.DvcbID == "HN")
        ...
        #region VI_DinhMuc        
        public static void VINapNLDinhMuc(CoBaoGA rowCoBao, List<CoBaoGACT> listcobaoct, List<DoanThongGACT> listdoanthongct)
        {
            try
            {
                if (rowCoBao.DvcbID == "VIN")
                {
                    ...
                    foreach (DoanThongGACT ct in listdoanthongct)
                    {
                        ...
                        else if(ct.TinhChatID==4)
                        {
                            ...
                            var dmGH = HttpHelper.Get<VINLDinhMuc>(Configuration.UrlCBApi + "api/Vinhs/VIGetNLDinhMucOBJ" + data).Result;
                            if (dmGH != null)
                            {
                                ct.DinhMuc += (decimal)dmGH.DinhMuc;
                                if (!string.IsNullOrWhiteSpace(ct.DienGiai)) ct.DienGiai += "-";
                                ct.DienGiai += "Ghép nguội=" + dmGH.DinhMuc + " " + dmGH.DonVi;
                            }
                        }    
                        //Nếu là máy đẩy
        ...
        private static void NapDinhMucMayChinh(CoBaoGA rowCoBao,string khuDoan, DoanThongGACT ct, int sumDon,decimal kmDM,decimal tyleDay, decimal tanDT)
        {
            string[] strArrayMTU = { "D13E-716","D13E-720", "D13E-722", "D13E-725" };
            string loaiMay = strArrayMTU.Contains(rowCoBao.DauMayID) ? rowCoBao.LoaiMayID + "-MTU" : rowCoBao.LoaiMayID;
            //Nạp định mức
        ...
        public static void VINapDMDinhMuc(CoBaoGA rowCoBao, decimal kmTotal, List<DoanThongGADM> listdoanthongdm)
        {
            try
            {
                if (rowCoBao.DvcbID == "VIN")
        ...
        #region DN_DinhMuc        
        public static void DNNapNLDinhMuc(CoBaoGA rowCoBao, List<DoanThongGACT> listdoanthongct)
        {
            try
            {
        ...
        #region SG_DinhMuc        
        public static void SGNapNLDinhMuc(CoBaoGA rowCoBao, List<CoBaoGACT> listcobaoct, List<DoanThongGACT> listdoanthongct)
        {
            try
            {
