...
{
    public partial class DNKhuDoanForm : DevComponents.DotNetBar.Metro.MetroForm
    {       
        bool bThem = false;
        public DNKhuDoanForm()
        {
        ...
        private void fnTraTim()
        {
            try
            {
                ...
                List<DNKhuDoan> listDNKhuDoan = HttpHelper.GetList<DNKhuDoan>(Configuration.UrlCBApi + "api/DaNangs/DNGetKhuDoan" + data)
                   .OrderBy(x=>x.KhuDoanID).ToList();                
                if (listDNKhuDoan.Count <= 0)
                {
                    throw new Exception("Không có dữ liệu.");

                }
                bsKhuDoan.DataSource = listDNKhuDoan;
                dataGridView1.Refresh();
                lblTableCount.Text = "Tổng số bản ghi:" + listDNKhuDoan.Count.ToString("N0");
        ...
        {
            DNKhuDoan kd = bsKhuDoan.Current as DNKhuDoan;
            if (kd != null)
            {
        ...
        private DNKhuDoan BindObject()
        {
            DNKhuDoan kd = new DNKhuDoan();
            if (!bThem)
                kd = bsKhuDoan.Current as DNKhuDoan;
        ...
        private void TinhCacGa_KM(DNKhuDoan kd)
        {
            kd.Km = 0;
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                var opStatus = HttpHelper.Delete<DNKhuDoan>(Configuration.UrlCBApi + "api/DaNangs/DNDeleteKhuDoan?id=" + kd.ID);
        ...
        private async void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bThem)
                {
                    ...
                    var objInsert = await HttpHelper.Post<DNKhuDoan>(Configuration.UrlCBApi + "api/DaNangs/DNPostKhuDoan", kd);                   
                    kd.ID = objInsert.ID;
                    bsKhuDoan.Add(kd);
                    bsKhuDoan.MoveLast();
                }
                else
                {
                    ...
                    kd.Modifyby = AppGlobal.User.Username;
                    kd.ModifyName = AppGlobal.User.FullName;
                    var objUpdate = await HttpHelper.Put<DNKhuDoan>(Configuration.UrlCBApi + "api/DaNangs/DNPutKhuDoan?id=" + kd.ID, kd);
                    bsKhuDoan.EndEdit();
                }
            }
            catch (Exception ex)
