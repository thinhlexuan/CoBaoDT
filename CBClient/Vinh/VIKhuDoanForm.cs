...
{
    public partial class VIKhuDoanForm : DevComponents.DotNetBar.Metro.MetroForm
    {       
        bool bThem = false;
        public VIKhuDoanForm()
        {
        ...
        private void fnTraTim()
        {
            try
            {
                ...
                List<VIKhuDoan> listVinhKhuDoan = HttpHelper.GetList<VIKhuDoan>(Configuration.UrlCBApi + "api/Vinhs/VIGetKhuDoan" + data)
                   .OrderBy(x=>x.KhuDoanID).ToList();                
                if (listVinhKhuDoan.Count <= 0)
                {
                    throw new Exception("Không có dữ liệu.");

                }
                bsKhuDoan.DataSource = listVinhKhuDoan;
                dataGridView1.Refresh();
                lblTableCount.Text = "Tổng số bản ghi:" + listVinhKhuDoan.Count.ToString("N0");
        ...
        private VIKhuDoan BindObject()
        {
            VIKhuDoan kd = new VIKhuDoan();
            if (!bThem)
                kd = bsKhuDoan.Current as VIKhuDoan;
        ...
        private void TinhCacGa_KM(VIKhuDoan kd)
        {
            kd.Km = 0;
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                var opStatus = HttpHelper.Delete<VIKhuDoan>(Configuration.UrlCBApi + "api/Vinhs/VIDeleteKhuDoan?id=" + kd.ID);
        ...
        private async void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bThem)
                {
                    ...
                    var objInsert = await HttpHelper.Post<VIKhuDoan>(Configuration.UrlCBApi + "api/Vinhs/VIPostKhuDoan", kd);                   
                    kd.ID = objInsert.ID;
                    bsKhuDoan.Add(kd);
                    bsKhuDoan.MoveLast();
                }
                else
                {
                    ...
                    kd.Modifyby = AppGlobal.User.Username;
                    kd.ModifyName = AppGlobal.User.FullName;
                    var objUpdate = await HttpHelper.Put<VIKhuDoan>(Configuration.UrlCBApi + "api/Vinhs/VIPutKhuDoan?id=" + kd.ID, kd);
                    bsKhuDoan.EndEdit();
                }
            }
            catch (Exception ex)
