...
{
    public partial class BCCTGioDonForm : DevComponents.DotNetBar.Metro.MetroForm
    {   
        string loaiBC = string.Empty;
        List<DonViKT> DonviKTList = new List<DonViKT>();
        public BCCTGioDonForm()
        {
            ...
            Library.FormHelper.AddEnterKeyPressAsTabEventHandler(this);
             DonviKTList = HttpHelper.GetList<DonViKT>(Configuration.UrlCBApi + "api/DanhMucs/GetDonViKT");
            var listKT = (from x in DonviKTList group x by new { x.MaDVCha } into g select new { TenDVCha = g.Key.MaDVCha }).ToList();          
            cboDonViKT.Items.Add("Tất cả");
            foreach( var vdKT in listKT)
            {
                cboDonViKT.Items.Add(vdKT.TenDVCha);
            }    
            cboDonViKT.SelectedIndex = 0;    
        ...
        private void BCGioDonForm_Load(object sender, EventArgs e)
        {
        ...
        private void btnTraTim_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string donviKT = cboDonViKT.Text;               
                int nguonDL = cboNguondl.SelectedIndex;
                List<BCCTGioDonInfo> list = new List<BCCTGioDonInfo>();                
                int TongSoBG = 0;
                //Lấy dữ liệu
                BaoCaoDAO.NapBCCTGioDon(nguonDL,maDV, donviKT, cboLoaiBC.SelectedIndex, sdNgayBD.Value, sdNgayKT.Value, DonviKTList, ref TongSoBG, ref list);                
                ...
                FormHelper.ShowReport(reportViewer1, "RptGioDonCT", "BaoCaoDS", list, rptParamList);
