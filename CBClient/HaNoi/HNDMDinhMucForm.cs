...
{
    public partial class HNDMDinhMucForm : DevComponents.DotNetBar.Metro.MetroForm
    {       
        bool bThem = false;
        public HNDMDinhMucForm()
        {
        ...
        private void fnTraTim()
        {
            try
            {
               ...
               var query = HttpHelper.GetList<HNDMDinhMuc>(Configuration.UrlCBApi + "api/HaNois/HNGetDMDinhMuc" + data)
                ...
                List<HNDMDinhMuc> listHNDMDinhMuc=(from x in query
                                                     group x by new { x.LoaiMayID, x.DauMoID } into g
                                                     select new HNDMDinhMuc
                                                     {
                                                         ID = g.LastOrDefault().ID,                                                         
                                                         LoaiMayID = g.Key.LoaiMayID,
                                                         DauMoID = g.Key.DauMoID,
                                                         DauMoName = g.LastOrDefault().DauMoName,
                ...
                bsDMDinhMuc.DataSource = listHNDMDinhMuc;
                dataGridView1.Refresh();
                lblTableCount.Text = "Tổng số bản ghi:" + listHNDMDinhMuc.Count.ToString("N0");
        ...
        {
            HNDMDinhMuc dm = bsDMDinhMuc.Current as HNDMDinhMuc;
            if (dm != null)
            {
        ...
        private HNDMDinhMuc BindObject()
        {
            HNDMDinhMuc dm = new HNDMDinhMuc();
            if (!bThem)
                dm = bsDMDinhMuc.Current as HNDMDinhMuc;
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                var opStatus = HttpHelper.Delete<HNDMDinhMuc>(Configuration.UrlCBApi + "api/HaNois/HNDeleteDMDinhMuc?id=" + dm.ID);
        ...
        private async void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bThem)
                {
                    ...
                    var objInsert = await HttpHelper.Post<HNDMDinhMuc>(Configuration.UrlCBApi + "api/HaNois/HNPostDMDinhMuc", dm);                    
                    dm.ID = objInsert.ID;
                    bsDMDinhMuc.Add(dm);
                    bsDMDinhMuc.MoveLast();
                }
                else
                {
                    ...
                    dm.Modifyby = AppGlobal.User.Username;
                    dm.ModifyName = AppGlobal.User.FullName;
                    var objUpdate = await HttpHelper.Put<HNDMDinhMuc>(Configuration.UrlCBApi + "api/HaNois/HNPutDMDinhMuc?id=" + dm.ID, dm);
                    bsDMDinhMuc.EndEdit();
                }
            }
            catch (Exception ex)
